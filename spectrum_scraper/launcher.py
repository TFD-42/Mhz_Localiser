...
def mode_list(rows: list[Allocation]) -> None:
    ...
    country = input("  Filter country (blank = all, e.g. USA, ITU, FRA) : ").strip().upper()
    region = input("  Filter region  (blank = all)                       : ").strip()
...
def mode_lookup(rows: list[Allocation]) -> None:
    ...
    try:
        ...
        return
    country = input("  Filter country (blank = all)        : ").strip().upper()
